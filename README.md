...
- Keychain의 토큰은 **읽기**와, 만료가 임박했을 때의 **갱신**에만 씁니다. 로그인·로그아웃은
  하지 않습니다.
  - access token은 수명이 약 8시간이라, 갱신하지 않으면 CLI를 쓰지 않는 동안 사용량이
    끊깁니다. 그래서 만료 60초 전부터 앱이 refresh token으로 직접 갱신해 Keychain에 되씁니다
    (Claude Code는 300초 전부터 갱신하므로 CLI가 떠 있으면 CLI에 양보합니다).
  - 되쓰기 직전에 Keychain을 다시 읽어, 그 사이 CLI가 갱신했으면 저장된 값을 따릅니다.
    갱신이나 쓰기에 실패하면 **기존 자격증명을 건드리지 않습니다.**
  - Claude Code와 같은 `security` 경유로 쓰며, 값은 인자가 아니라 **stdin**으로 넘겨
    `ps`에 토큰이 노출되지 않게 합니다. 앱 시작 시 일회용 항목에 값을 실제로 써 보고 되읽어
    전달 방식을 정하며, 어떤 방식도 값을 온전히 저장하지 못하면 갱신 없이 읽기 전용으로
    동작합니다.
