...
- 최근 성공한 사용량 수치(토큰 제외)는 `~/Library/Caches/claude-rings/last-usage.json`에 캐시됩니다. 앱 재시작 후 새 조회가 성공하기 전까지 캐시된 값을 50% 투명도로 `stale` 상태로 표시합니다. (HTTP 429 차단 후 재시작하면 값이 없을 때도 있으므로, 이전 값을 임시로 보여줍니다)
