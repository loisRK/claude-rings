...
| 조회 일시 제한(429, retry-after) | 429 응답에 `Retry-After`가 있어 해당 시간까지 재조회하지 않음 | 이전 값 없으면 메뉴바에 🕐 + 남은 시간(예: "59m"). 이전 값 있으면 흐린 숫자 그대로 표시하고 팝오버에 "조회 일시 제한 · N 후 재시도" |
...
- `.variableLength`는 버튼 안 커스텀 SwiftUI 서브뷰의 폭을 자동으로 반영하지
  않으므로, `NSHostingView.fittingSize`로 계산한 값을 `statusItem.length`에 직접
  설정합니다(계정 목록은 시작 시 1회만 로드돼 실행 중 바뀌지 않으므로 한 번이면 됨).
...
- 게이지 색·채움 비율은 Session·Weekly 중 **더 낮은 쪽**(min)을 기준으로 합니다. 두
  ...
  각자의 값 그대로).
- 값이 없으면 `—`, 조회 일시 제한 중이면 `🕐 59m`(clock 심볼 + 남은 시간)로 대체합니다.
...
### 팝오버(`NSPopover`)
- 메뉴바 항목을 클릭하면 열고(`behavior = .transient`), 다시 클릭하거나 바깥을
  누르면 닫힙니다.
- 계정마다 세로로 나열:
  - 계정 이름, 실행 중이면 ● 표시
